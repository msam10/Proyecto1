...
function App() {
  ...
  const [loading, setLoading] = useState(false); // Cambiado a minúscula por convención
  ...
  const addCarta = async (personaje: IPersonaje) => {
    ...
    try {
      ...
      await fetchTask(); // Refrescamos la lista
    } catch (e) {
      console.error("Error adding task", e);
    } finally {
      setLoading(false); // Apagamos carga
    }
  };
  ...
  return (
    <div className="min-h-screen bg-linear-to-br from-amber-200 via-sky-200 to-amber-300 flex flex-col items-center py-12">
      ...
      {/* OVERLAY DE CARGA (Opcional pero recomendado) */}
      {loading && (
